...
const GroupJoined = () => {
  ...
  // *** Query
  const {data: groupJoineds} = GroupQuery.useListGroupJoined(userLogin?.uid, {
    ...query,
    search: debouncedValue,
  })

  if (!groupJoineds || (groupJoineds && groupJoineds.data.length === 0))
    return (
      <Alert color="warning">
        <div className="alert-body">Bạn chưa tham gia học tập nào </div>
      </Alert>
    )
  ...
  return (
    <div>
      ...
      />
      <Row className="mt-3">
        {groupJoineds.data.map(item => (
          <Col lg="4" md="6" className="mb-3" sm="12" key={item._id}>
            <GroupItem item={item} type="joined" />
          </Col>
        ))}
      </Row>
      <div>
        <ReactPaginate
          ...
          pageCount={
            groupJoineds?.metadata?.total_page ||
            groupJoineds?.metadata?.num_pages ||
            1
          }
          breakLabel="..."
          ...
          pageLinkClassName={`page-link ${
            groupJoineds?.metadata?.total_page > 1000 ? 'mx-3 p-5' : ''
          }`}
