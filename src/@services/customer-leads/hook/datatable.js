...
const get_list_tag_api = PROXY + '/config/tags'
/**
 * helper function
 */
...
const useGetListCustomerLeadAssigned = uid => {
  return {
    ...optionsCallApi(PROXY + `/general/get-list-customer/${uid}`),
    key: [keys.LIST_CUSTOMER_LEAD_ASSIGNED, uid],
  }
}

const useListTagsCustomerLeadDatatable = () => {
  return {
    ...optionsCallApi(get_list_tag_api),
    key: [keys.LIST_TAGS_CUSTOMER_LEAD],
  }
}
...
const ExploiteCustomerLeadsDatatable = {
  ...
  useListUidExploited,
  useGetListCustomerLeadAssigned,
  useListTagsCustomerLeadDatatable,
}
