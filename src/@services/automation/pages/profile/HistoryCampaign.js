...
// *** Query
import {CampaignQuery} from '@services/campaign'
...
const HistoryCampaign = ({data}) => {
  // *** react hook
  const {id} = useParams()
  const campaignId = data?._id || ''

  const {data: histories} = CampaignQuery.useGetDetailCampaignHistoryV2(
    campaignId,
    id,
  )

  console.log('HISTORY CAMPAIGN', histories)
