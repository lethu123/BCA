      ...
      return (
        ...
        >
          <div className="d-flex align-items-center">
            ...
            />
            <span className="font-weight-bold ml-75 mr-1">{item.name}</span>
            <span>- {item.usage}%</span>
          </div>
          <div>
            <span>{item.upDown}%</span>
            {item.upDown > 0 ? (
              <Icon.ArrowUp size={14} className="ml-1 text-success" />
            ) : (
              <Icon.ArrowDown size={14} className="ml-1 text-danger" />
            )}
          </div>
        </div>
      )
