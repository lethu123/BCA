...
export const columns = [
  ...
  {
    name: 'UID FanPage',
    selector: 'impact_fanpage_link_info',
    sortable: true,
    minWidth: '170px',
    cell: row => (
      <div className="d-flex align-items-center">
        <img
          src={row.impact_fanpage_link_info.avatar || avatar}
          alt="images"
          className="img-fluid rounded-circle mr-2"
          width="30px"
          height="30px"
        ...
        <div className="user-info text-truncate ml-1">
          <Link
            className="d-block  text-truncate text-primary cursor-pointer"
            to={row.impact_fanpage_link_info.link || '#'}
          >
            {row.impact_fanpage_link_info.name}
          </Link>
        </div>
      </div>
    ),
  ...
  {
    name: 'Loại Page',
    selector: 'type_fanpage',
    sortable: true,
    minWidth: '100px',
    cell: row => (
      <span className="d-block  text-truncate">{row.type_fanpage}</span>
    ),
  },
  {
    name: 'Chiến dịch',
